function drop() {
    document.getElementById("dropdownMenuLink").style.backgroundColor = "#205720";
}

function registro() {
    document.getElementById("conect").style.display = "none";
    document.getElementById("registro").style.display = "block";
    document.getElementById("inicio-p").style.backgroundColor = "#369741";
    document.getElementById("inicio-p").style.color = "#000000";

    document.getElementById("registro-p").style.backgroundColor = "#093a0d";
    document.getElementById("registro-p").style.color = "#ffffff";

}

function inicio() {
    document.getElementById("conect").style.display = "block";
    document.getElementById("registro").style.display = "none";
    document.getElementById("inicio-p").style.backgroundColor = "#093a0d";
    document.getElementById("inicio-p").style.color = "#ffffff";

    document.getElementById("registro-p").style.backgroundColor = "#369741";

    document.getElementById("registro-p").style.color = "#000000";
}


 /* todos */

function directo() {
    document.getElementById("hoy").style.display = "block";
    document.getElementById("finalizados").style.display = "none";
    document.getElementById("proximos").style.display = "none";
}


function finalizado() {
    document.getElementById("hoy").style.display = "none";
    document.getElementById("finalizados").style.display = "block";
    document.getElementById("proximos").style.display = "none";
}


function proximo() {
    document.getElementById("hoy").style.display = "none";
    document.getElementById("finalizados").style.display = "none";
    document.getElementById("proximos").style.display = "block";

}



/*  futbol */
function directofutbol() {
    document.getElementById("hoyfutbol").style.display = "block";
    document.getElementById("finalizadosfutbol").style.display = "none";
    document.getElementById("proximosfutbol").style.display = "none";
}


function finalizadofutbol() {
    document.getElementById("hoyfutbol").style.display = "none";
    document.getElementById("finalizadosfutbol").style.display = "block";
    document.getElementById("proximosfutbol").style.display = "none";
}


function proximofutbol() {
    document.getElementById("hoyfutbol").style.display = "none";
    document.getElementById("finalizadosfutbol").style.display = "none";
    document.getElementById("proximosfutbol").style.display = "block";

}


 /* basket */
 function directobasket() {
    document.getElementById("hoybasket").style.display = "block";
    document.getElementById("finalizadosbasket").style.display = "none";
    document.getElementById("proximosbasket").style.display = "none";
}


function finalizadobasket() {
    document.getElementById("hoybasket").style.display = "none";
    document.getElementById("finalizadosbasket").style.display = "block";
    document.getElementById("proximosbasket").style.display = "none";
}


function proximobasket() {
    document.getElementById("hoybasket").style.display = "none";
    document.getElementById("finalizadosbasket").style.display = "none";
    document.getElementById("proximosbasket").style.display = "block";

}





 /* laliga */

 function directolaliga() {
    document.getElementById("hoylaliga").style.display = "block";
    document.getElementById("finalizadoslaliga").style.display = "none";
    document.getElementById("proximoslaliga").style.display = "none";
}


function finalizadolaliga() {
    document.getElementById("hoylaliga").style.display = "none";
    document.getElementById("finalizadoslaliga").style.display = "block";
    document.getElementById("proximoslaliga").style.display = "none";
}


function proximolaliga() {
    document.getElementById("hoylaliga").style.display = "none";
    document.getElementById("finalizadoslaliga").style.display = "none";
    document.getElementById("proximoslaliga").style.display = "block";

}


/* premier */

function directopremier() {
    document.getElementById("hoypremier").style.display = "block";
    document.getElementById("finalizadospremier").style.display = "none";
    document.getElementById("proximospremier").style.display = "none";
}


function finalizadopremier() {
    document.getElementById("hoypremier").style.display = "none";
    document.getElementById("finalizadospremier").style.display = "block";
    document.getElementById("proximospremier").style.display = "none";
}


function proximopremier() {
    document.getElementById("hoypremier").style.display = "none";
    document.getElementById("finalizadospremier").style.display = "none";
    document.getElementById("proximospremier").style.display = "block";

}


/* PRIMERA URUGUAY */

function directoprimerauy() {
    document.getElementById("hoyprimerauy").style.display = "block";
    document.getElementById("finalizadosprimerauyr").style.display = "none";
    document.getElementById("proximosprimerauy").style.display = "none";
}


function finalizadoprimerauy() {
    document.getElementById("hoyprimerauy").style.display = "none";
    document.getElementById("finalizadosprimerauyr").style.display = "block";
    document.getElementById("proximosprimerauy").style.display = "none";
}


function proximoprimerauy() {
    document.getElementById("hoyprimerauy").style.display = "none";
    document.getElementById("finalizadosprimerauyr").style.display = "none";
    document.getElementById("proximosprimerauy").style.display = "block";

}

/* PRIMERA ARGENTINA */

function directoprimeraARG() {
    document.getElementById("hoyprimeraARG").style.display = "block";
    document.getElementById("finalizadosprimeraARG").style.display = "none";
    document.getElementById("proximosprimeraARG").style.display = "none";
}


function finalizadoprimeraARG() {
    document.getElementById("hoyprimeraARG").style.display = "none";
    document.getElementById("finalizadosprimeraARG").style.display = "block";
    document.getElementById("proximosprimeraARG").style.display = "none";
}


function proximoprimeraARG() {
    document.getElementById("hoyprimeraARG").style.display = "none";
    document.getElementById("finalizadosprimeraARG").style.display = "none";
    document.getElementById("proximosprimeraARG").style.display = "block";

}

/* SERIE A */

function directoserieA() {
    document.getElementById("hoyserieA").style.display = "block";
    document.getElementById("finalizadosserieA").style.display = "none";
    document.getElementById("proximosserieA").style.display = "none";
}


function finalizadoserieA() {
    document.getElementById("hoyserieA").style.display = "none";
    document.getElementById("finalizadosserieA").style.display = "block";
    document.getElementById("proximosserieA").style.display = "none";
}


function proximoserieA() {
    document.getElementById("hoyserieA").style.display = "none";
    document.getElementById("finalizadosserieA").style.display = "none";
    document.getElementById("proximosserieA").style.display = "block";

}

/* tenis */

function directoTenis() {
    document.getElementById("hoyTenis").style.display = "block";
    document.getElementById("finalizadosTenis").style.display = "none";
    document.getElementById("proximosTenis").style.display = "none";
}


function finalizadoTenis() {
    document.getElementById("hoyTenis").style.display = "none";
    document.getElementById("finalizadosTenis").style.display = "block";
    document.getElementById("proximosTenis").style.display = "none";
}


function proximosTenis() {
    document.getElementById("hoyTenis").style.display = "none";
    document.getElementById("finalizadosTenis").style.display = "none";
    document.getElementById("proximosTenis").style.display = "block";

}

/* NBA ESTE */

function directoNBAeste() {
    document.getElementById("hoyNBAeste").style.display = "block";
    document.getElementById("finalizadosNBAeste").style.display = "none";
    document.getElementById("proximosNBAeste").style.display = "none";
}


function finalizadosNBAeste() {
    document.getElementById("hoyNBAeste").style.display = "none";
    document.getElementById("finalizadosNBAeste").style.display = "block";
    document.getElementById("proximosNBAeste").style.display = "none";
}


function proximosNBAeste() {
    document.getElementById("hoyNBAeste").style.display = "none";
    document.getElementById("finalizadosNBAeste").style.display = "none";
    document.getElementById("proximosNBAeste").style.display = "block";

}

/* NBA OESTE */

function directoNBAoeste() {
    document.getElementById("hoyNBAoeste").style.display = "block";
    document.getElementById("finalizadosNBAoeste").style.display = "none";
    document.getElementById("proximosNBAoeste").style.display = "none";
}


function finalizadoNBAoeste() {
    document.getElementById("hoyNBAoeste").style.display = "none";
    document.getElementById("finalizadosNBAoeste").style.display = "block";
    document.getElementById("proximosNBAoeste").style.display = "none";
}


function proximoNBAoeste() {
    document.getElementById("hoyNBAoeste").style.display = "none";
    document.getElementById("finalizadosNBAoeste").style.display = "none";
    document.getElementById("proximosNBAoeste").style.display = "block";

}

/* LUB */

function directoLUB() {
    document.getElementById("hoyLUB").style.display = "block";
    document.getElementById("finalizadosLUB").style.display = "none";
    document.getElementById("proximosLUB").style.display = "none";
}


function finalizadoLUB() {
    document.getElementById("hoyLUB").style.display = "none";
    document.getElementById("finalizadosLUB").style.display = "block";
    document.getElementById("proximosLUB").style.display = "none";
}


function proximoLUB() {
    document.getElementById("hoyLUB").style.display = "none";
    document.getElementById("finalizadosLUB").style.display = "none";
    document.getElementById("proximosLUB").style.display = "block";

}

/**********/
/* US OPEN */

function directoUSOPEN() {
    document.getElementById("hoyUSOPEN").style.display = "block";
    document.getElementById("finalizadosUSOPEN").style.display = "none";
    document.getElementById("proximosUSOPEN").style.display = "none";
}


function finalizadoUSOPEN() {
    document.getElementById("hoyUSOPEN").style.display = "none";
    document.getElementById("finalizadosUSOPEN").style.display = "block";
    document.getElementById("proximosUSOPEN").style.display = "none";
}


function proximoUSOPEN() {
    document.getElementById("hoyUSOPEN").style.display = "none";
    document.getElementById("finalizadosUSOPEN").style.display = "none";
    document.getElementById("proximosUSOPEN").style.display = "block";

}

/* wimbledon */

function directoWimbledon() {
    document.getElementById("hoyWimbledon").style.display = "block";
    document.getElementById("finalizadosWimbledon").style.display = "none";
    document.getElementById("proximosWimbledon").style.display = "none";
}


function finalizadoWimbledon() {
    document.getElementById("hoyWimbledon").style.display = "none";
    document.getElementById("finalizadosWimbledon").style.display = "block";
    document.getElementById("proximosLUB").style.display = "none";
}


function proximoWimbledon() {
    document.getElementById("hoyWimbledon").style.display = "none";
    document.getElementById("finalizadosWimbledon").style.display = "none";
    document.getElementById("proximosWimbledon").style.display = "block";

}

/* roland garros */

function directoRoland() {
    document.getElementById("hoyRoland").style.display = "block";
    document.getElementById("finalizadosRoland").style.display = "none";
    document.getElementById("proximosRoland").style.display = "none";
}


function finalizadoRoland() {
    document.getElementById("hoyRoland").style.display = "none";
    document.getElementById("finalizadosRoland").style.display = "block";
    document.getElementById("proximosRoland").style.display = "none";
}


function proximoRoland() {
    document.getElementById("hoyRoland").style.display = "none";
    document.getElementById("finalizadosRoland").style.display = "none";
    document.getElementById("proximosRoland").style.display = "block";

}

/* australain */

function directoAUSOPEN() {
    document.getElementById("hoyAUSOPEN").style.display = "block";
    document.getElementById("finalizadosAUSOPEN").style.display = "none";
    document.getElementById("proximosAUSOPEN").style.display = "none";
}


function finalizadoAUSOPEN() {
    document.getElementById("hoyAUSOPEN").style.display = "none";
    document.getElementById("finalizadosAUSOPEN").style.display = "block";
    document.getElementById("proximosAUSOPEN").style.display = "none";
}


function proximoAUSOPEN() {
    document.getElementById("hoyAUSOPEN").style.display = "none";
    document.getElementById("finalizadosAUSOPEN").style.display = "none";
    document.getElementById("proximosAUSOPEN").style.display = "block";

}

/* fav */

function directoFAV() {
    document.getElementById("hoyfav").style.display = "block";
    document.getElementById("finalizadosFAV").style.display = "none";
    document.getElementById("proximosFav").style.display = "none";
}


function finalizadoFAV() {
    document.getElementById("hoyfav").style.display = "none";
    document.getElementById("finalizadosFAV").style.display = "block";
    document.getElementById("proximosFav").style.display = "none";
}


function proximoFAV() {
    document.getElementById("hoyfav").style.display = "none";
    document.getElementById("finalizadosFAV").style.display = "none";
    document.getElementById("proximosFav").style.display = "block";

}

/* datos personales */

function datospersonales() {
    document.getElementById("personales").style.display = "block";
    document.getElementById("favperonal").style.display = "none";
    document.getElementById("selecperosnal").style.display = "none";
}


function favoritossss() {
    document.getElementById("personales").style.display = "none";
    document.getElementById("favperonal").style.display = "block";
    document.getElementById("selecperosnal").style.display = "none";
}


function selecfav() {
    document.getElementById("personales").style.display = "none";
    document.getElementById("favperonal").style.display = "none";
    document.getElementById("selecperosnal").style.display = "block";

}